...
export const WeddingGift = ({ config }: Props) => (
  <section className="w-full max-w-lg mb-20 px-4">
    ...
    <motion.div
      className="bg-[color-mix(in_srgb,var(--bg)_80%,black)] border border-[var(--accent)]/40 rounded-2xl p-5 shadow-xl text-center"
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
    >
      ...
      <p className="text-sm text-[var(--text)]/90 mb-4">
        A/n {config.accountName}
      </p>

      <button
        onClick={() => navigator.clipboard.writeText(config.accountNumber)}
        className="bg-[var(--accent)] text-[var(--bg)] px-5 py-2 rounded-full font-[var(--font-header)] hover:opacity-90"
      >
        Copy Account Number
      </button>
